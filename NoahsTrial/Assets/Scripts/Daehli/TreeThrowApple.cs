...
public class TreeThrowApple : MonoBehaviour {
	...
	void Update () {
		...
		if (Vector2.Distance(instObject.transform.position, instObject.GetComponent<appleManager>().initPosition)> speed) {
			Debug.Log ("Distance: " + Vector2.Distance (instObject.transform.position, instObject.GetComponent<appleManager> ().initPosition));
	...
	IEnumerator throwApple(){
		for (int i = 0; i < 100; i++) {
			if (i % 2 == 0) {
				instObject  = (GameObject)Instantiate (apple, direction, Quaternion.identity);
				instObject.GetComponent<appleManager> ().initPosition = direction;
